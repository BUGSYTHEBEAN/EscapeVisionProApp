...
public struct GestureComponent: Component, Codable {
    ...
    /// A Boolean value that indicates whether the entity should have gravity after drag.
    public var addGravity: Bool = true
    ...
    mutating func onChanged(value: EntityTargetValue<DragGesture.Value>) {
        ...
        if addGravity {
            if var physicsComponent = value.entity.components[PhysicsBodyComponent.self] {
                if (physicsComponent.isAffectedByGravity) {
    ...
    mutating func onEnded(value: EntityTargetValue<DragGesture.Value>) {
        ...
        if addGravity {
            if var physicsComponent = state.targetedEntity?.components[PhysicsBodyComponent.self] {
                physicsComponent.isAffectedByGravity = true
                state.targetedEntity?.components.set(physicsComponent)
            }
        }
